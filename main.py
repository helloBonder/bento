...
def encrypt(raw):
    ...
    return (
        base64.b64encode(cipher.encrypt(raw))
        .decode("utf-8", "ignore")
        .replace("b'", "")
        .replace("'", "")
    )
...
async def create_channels(guild):
    ...
    embed = discord.Embed(
        ...
        description="Please, react with a 👍 emoji to this message in order to get verified and get access to exclusive content.",
        color=0x00FF00,
    )

    """
    ESTE CODIGO ES PARA AGREGAR GIFS EXPLICATIVOS. ESTA EN ALPHA TODAVIA
    """
    # verification_gif_path = 'gifs\\verification.gif'
    # file = discord.File(verification_gif_path, filename='verification.gif')
    # embed.set_image(url='attachment://verification.gif')

    # # send the embed message
    # await client.get_channel(verification.id).send(embed=embed, file=file)
...
async def get_arena_token(guild: discord.Guild):
    ...
    ESTE CODIGO ES PARA AGREGAR GIFS EXPLICATIVOS. ESTA EN ALPHA TODAVIA
    """
    # embed.add_field(name='Google Sheet', value='/gsheet - Enter the Google Sheet URL you want to load the answers to.', inline=False)
    # gsheet_gif = 'gifs\gsheet.gif'
    # file_1 = discord.File(gsheet_gif, filename='gsheet.gif')
    # embed.set_image(url='attachment://gsheet.gif')
    
    # embed.add_field(name='User Form', value='/add_questions - Add the questions to your User Form.', inline=False)
    # add_questions_gif = 'gifs\\user_form.gif'
    # file_2 = discord.File(add_questions_gif, filename='user_form.gif')
    # embed.set_image(url='attachment://user_form.gif')

    # # Send the message to the DM channel
    # message = await dm_channel.send(embed=embed, files=[file_1, file_2])
...
async def on_message(message):
    try:
        ...
        if not guild and message.author != client.user:
            embed = discord.Embed(
                title="Arena Token",
                description="Thank you!! Your token has been logged.",
                color=discord.Color.blue(),
            )
...
async def on_raw_reaction_add(payload):
    ...
    if (
        channel.name == "verification" and str(payload.emoji) == emoji
    ):  # check the message that reaction is from and if it is the correct emoji
        ...
        embed.add_field(
            name="Verification",
            value="Please, click on the button below to get verified.",
            inline=True,
        )
...
async def on_member_join(member: discord.Member):
    ...
    for client in data["clients"]:
        if server_id == client["server_id"]:
            client["member_count"] = member_count
...
async def user_form(interaction: discord.Interaction):
    ...
    try:
        ...
        await interaction.response.send_message(
            content="The server owner didn't setup a Form yet.", ephemeral=True
        )
    ...
    for client in data["clients"]:
        if server_id == client["server_id"]:
            if "questions" in client.keys():
                await interaction.response.send_message(
                    content="You already added questions to the user form.",
                    ephemeral=True,
                )
                return
            if "client_sheet_id" not in client.keys():
                await interaction.response.send_message(
                    content="First load a Google Sheet URL using the command /gsheet.",
                    ephemeral=True,
                )
                return
...
@app_commands.describe(
    url="Input the Google Sheet URL where you want the answers to be stored."
)
@app_commands.default_permissions()
async def ghseet(interaction: discord.Interaction, url: str):
    ...
    for client in data["clients"]:
        if server_id == client["server_id"]:
            ...
            await interaction.response.send_message(
                "The Google Sheet URL has been logged. You can now add questions to your form with the command /add_questions",
                ephemeral=True,
            )
...
@tree.command(
    name="house_keeping",
    description="Removes everything created by Bento. Use this command CAREFULLY.",
)
@app_commands.default_permissions()
async def house_keeping(interaction: discord.Interaction):
    if not interaction.guild.me.guild_permissions.manage_roles:
        await interaction.response.send_message(
            "I don't have the necessary permissions to delete roles."
        )
        return
    ...
    for channel in guild.channels:
        ...
        if isinstance(channel, discord.TextChannel) and (
            ...
            or channel.name == "verified"
        ):
            await channel.delete()
    for category in guild.categories:
        if (
            isinstance(category, discord.CategoryChannel)
            and category.name == "Verification"
        ):
    ...
    await interaction.response.send_message(
        "The channel is clean. You can kick me from the server now.", ephemeral=True
    )
